...
void __fastcall MSExcelWorks::SaveDocument(Variant& workbook, const AnsiString& FileName)
{
    ...
    {
        try 
        {  // Сохранение ранее открытого документа
...
Variant MSExcelWorks::ExportToExcelTable(TDataSet* QTable, Variant Worksheet, String RangeName, bool fUnbounded)
{
    ...
    if (RecordCount > RangeRowsCount)
    {
        if (fUnbounded)
        {
            ...
            int rowHeight = GetRowHeight(range_body);   // Копируем высоту ячеек (почему не копируется CopyRangeFormat)
    ...
    std::vector< std::pair<String, bool> > bindingList; // Первый параметр -  имя поля, второй - признак успешного поиска
    ...
    for (int i = 1; i <= RangeColumnsCount; i++)
    {
        ...
        /*
        if (field)     // Проверяем, существует ли поле с этим именем в нашем источнике
        {
